...
class Newappointmentcontroller extends GetxController {
  ...
  var branches = <BranchModel>[].obs; // assume Branch is your model
  ...
  Future<void> fetchServicesForBranch() async {
    try {
      ...
      var response = await dioClient.getData(
        '${Apis.baseUrl}/services?salon_id=${loginUser!.salonId}',
        (json) => json,
      );
  ...
  Future<void> fetchStaffsForBranch() async {
    try {
      final loginUser = await prefs.getUser();
      var response = await dioClient.getData(
        '${Apis.baseUrl}/staffs?salon_id=${loginUser!.salonId}',
        (json) => json,
      );
  ...
  void nextStep() {
    if (currentStep.value < 5) {
      currentStep.value++;
      // Auto-open date picker when reaching step 3 (Case 4)
  ...
  Future<void> getBranches() async {
    try {
      final loginUser = await prefs.getUser();
      var response = await dioClient.getData(
        '${Apis.baseUrl}/branches?salon_id=${loginUser!.salonId}',
        (json) => json,
      );
  ...
  Future<void> fetchAvailableSlots() async {
    ...
    try {
      ...
      var response = await dioClient.getData(
        '${Apis.baseUrl}/quick-booking/available-slots?salon_id=${loginUser!.salonId}&date=$dateStr&staff_id=${staff.id}',
        (json) => json,
      );
  ...
  List<StaffModel> get filteredStaff {
    ...
    return staff
        .where(
            (s) => s.branchId == branch.id && s.serviceIds.contains(service.id))
        .toList();
  }
